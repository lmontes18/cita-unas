...
<div class="ac-wrap">
  <div class="ac-inner">
    ...
    <div class="ac-header">
      <div class="ac-eyebrow">✦ Modificar Cita</div>
      <h2 class="ac-title">Editar Detalles</h2>
    </div>
      ...
      <form action="{{ route('appointments.update', $appointment->id) }}" method="POST">
        ...
        <div class="ac-row">
          <div class="ac-field">
            ...
            @php 
                $currentDate = \Carbon\Carbon::parse($appointment->start_time)->format('Y-m-d');
                $currentTime = \Carbon\Carbon::parse($appointment->start_time)->format('H:i');
            @endphp
            <input type="date" name="date" class="ac-input" 
                   value="{{ old('date', $currentDate) }}" required>
          </div>
          <div class="ac-field">
              ...
              <select name="time" class="ac-select" required>
                @for ($hour = 7; $hour <= 20; $hour++)
                  @foreach(['00', '30'] as $min)
                    @php $t = sprintf('%02d:%s', $hour, $min); @endphp
                    <option value="{{ $t }}" {{ old('time', $currentTime) == $t ? 'selected' : '' }}>
                        {{ $t }}
                    </option>
                  @endforeach
                @endfor
        ...
        <div class="ac-field">
          ...
          <textarea name="notes" class="ac-textarea">{{ old('notes', $appointment->notes) }}</textarea>
        </div>

        <div class="ac-actions">
          <a href="{{ route('appointments.index') }}" class="ac-cancel">Regresar</a>
...
<script>
// El mismo script que tienes en create, pero enviando el ID de la cita actual para ignorarla en la validación
document.addEventListener('DOMContentLoaded', function() {
    const dateInput = document.querySelector('input[name="date"]');
    const timeSelect = document.querySelector('select[name="time"]');
    const messageDiv = document.getElementById('availability-message');
    const submitBtn = document.querySelector('.ac-submit');
    const appointmentId = "{{ $appointment->id }}"; // ID de la cita actual
    ...
    function checkConflict() {
        ...
        if (date && time && services.length > 0) {
            fetch("{{ route('appointments.check') }}", {
                ...
                body: JSON.stringify({ date, time, services, appointment_id: appointmentId })
            })
    ...
    document.querySelectorAll('input[name="services[]"]').forEach(cb => cb.addEventListener('change', checkConflict));
});
</script>
</x-app-layout>